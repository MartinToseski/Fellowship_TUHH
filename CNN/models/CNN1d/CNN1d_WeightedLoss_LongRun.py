...
def run_experiment(config):
    ...
    checkpoint = ModelCheckpoint(monitor="val_f1_macro", mode="max", save_top_k=1, filename="{epoch}-{val_auc_macro:.4f}")
    early_stop = EarlyStopping(monitor="val_f1_macro", mode="max", patience=5, min_delta=0.001, verbose=True)

    trainer = pl.Trainer(max_epochs=config.max_epochs, logger=logger, callbacks=[checkpoint], devices=[0])
...
if __name__ == "__main__":
    ...
    for i in range(1):
        config = Config(
            model_name="1dCNN_NoEarlyStop_F1_SquaredLoss",
