...
def run_experiment(config):
    ...
    early_stop = EarlyStopping(monitor="val_auc_macro", mode="max", patience=10, min_delta=0.001, verbose=True)

    trainer = pl.Trainer(max_epochs=config.max_epochs, logger=logger, callbacks=[checkpoint, early_stop], devices=[0], gradient_clip_val=1.0)
