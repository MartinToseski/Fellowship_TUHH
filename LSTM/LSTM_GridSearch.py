...
def run_experiment(config):
    ...
    wandb_logger = WandbLogger(project="BiLSTM_GridSearch", entity="martintoseski13-kaunas-university-of-technology", name=version, log_model=True)
    ...
    early_stop = EarlyStopping(monitor="val_f1_macro", mode="max", patience=15, min_delta=1e-4, verbose=True)

    trainer = pl.Trainer(max_epochs=config.max_epochs, logger=[logger, wandb_logger], callbacks=[checkpoint, early_stop], gradient_clip_val=1.0, devices=[0])
